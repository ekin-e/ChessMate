...
class camera_XYZ:
    ...
    def find_board_coord(self, frame, x1, x2, y1, y2):
        ...
        print("corner original", corner_original)
...
def capture_and_detect():
    ...
    if new_frame is not None:
        ...
        corner_points_list = [middle_points_list[7][0],middle_points_list[7][7], middle_points_list[0][7], middle_points_list[0][0]]
        camera.points = np.array(corner_points_list, dtype=np.int32).reshape((-1, 2))
        results = model(new_frame, conf=0.65)
        camera.points[[2,3]] = camera.points[[3,2]]
        print("camera points:", camera.points)
